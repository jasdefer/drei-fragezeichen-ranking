...
def validate_ratings(ratings: List[Dict[str, str]], episodes: List[Dict[str, Any]]) -> None:
    ...
    for idx, rating in enumerate(ratings, start=2):  # Start bei 2 (Header ist Zeile 1)
        ...
        # Validierung: std_error muss eine nicht-negative Gleitkommazahl sein
        if std_error:
            try:
                std_error_float = float(std_error)
                if std_error_float < 0:
                    errors.append(
                        f"Zeile {idx} (ID: {episode_id}): "
                        f"std_error '{std_error}' darf nicht negativ sein"
                    )
            except ValueError:
                errors.append(
                    f"Zeile {idx} (ID: {episode_id}): "
                    f"std_error '{std_error}' ist keine gültige Gleitkommazahl"
                )
        else:
