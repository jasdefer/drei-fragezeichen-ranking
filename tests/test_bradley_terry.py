...
class TestBradleyTerry(unittest.TestCase):
    ...
    def test_happy_path_simple_preference(self):
        """
        ...
        self.assertAlmostEqual(mean_utility, 1.0, places=2, msg=f"Mean utility sollte ≈ 1.0 sein, ist {mean_utility}")
        
        # Episode 1 sollte höchste utility haben (stärkste Episode)
        self.assertGreater(utilities[1], utilities[2], "Episode 1 sollte stärker als Episode 2 sein")
        self.assertGreater(utilities[1], utilities[3], "Episode 1 sollte stärker als Episode 3 sein")
        
        # Episode 2 sollte stärker als Episode 3 sein (2 schlägt 3)
        self.assertGreater(utilities[2], utilities[3], "Episode 2 sollte stärker als Episode 3 sein")

    def test_connectivity_episode_1_present(self):
        """
        ...
        self.assertGreater(len(rating_rows), 0, "Rating-Rows sollten zurückgegeben werden")
        self.assertEqual(len(rating_rows), 3, "Alle 3 verbundenen Episoden sollten gerankt sein")

    def test_connectivity_episode_1_missing(self):
        """
        ...
        with self.assertRaisesRegex(BradleyTerryError, "Episode 1 ist nicht im Vergleichsgraph"):
            compute_ratings_from_polls(polls, calculated_at)

    def test_disconnected_episodes(self):
        """
        ...
        self.assertEqual(episode_ids, {1, 2}, f"Nur Episoden 1,2 sollten gerankt sein, gefunden: {episode_ids}")

    def test_stability_finite_values(self):
        """
    ...
    def test_empty_polls_list(self):
        ...
        self.assertEqual(rating_rows, [], "Leere Poll-Liste sollte leere Rating-Liste ergeben")

    def test_mean_utility_invariant(self):
        """
    ...
    def test_timezone_aware_required(self):
        ...
        with self.assertRaisesRegex(BradleyTerryError, "timezone-aware"):
            compute_ratings_from_polls(polls, naive_dt)

    def test_timezone_must_be_utc(self):
        """
        ...
        # Sollte Exception werfen
        with self.assertRaisesRegex(BradleyTerryError, "UTC timezone verwenden.*UTC offset = 0"):
